...
void 
Controller::config_gain(const Parameter_t::Gain& gain)
{
	Kp.setZero();
	Kv.setZero();
	Ka.setZero();
...
Eigen::Quaterniond 
Controller::computeDesiredAttitude(
...
Controller_Output_t 
Controller::computeNominalReferenceInputs(
    ...
    const Odom_Data_t& attitude_estimate) const
{
	...
	// 目标加速度
  	const Eigen::Vector3d des_acc = reference_state.a + Eigen::Vector3d(0, 0, param.gra);

  	// Reference attitude
  	const Eigen::Quaterniond q_W_B = computeDesiredAttitude(des_acc, 
	  	reference_state.yaw, attitude_estimate.q);
...
{
	// step1: Compute reference inputs
	...
	u.des_v_real = des.v + u_p;

	Eigen::Vector3d e_v = des.v + u_p - odom.v;
	...
	Eigen::Vector3d F_des = u_v * param.mass + Eigen::Vector3d(0.0, 0.0, param.mass * param.gra) + Ka * param.mass * des.a
						    - drag_accelerations * param.mass;
	...
	u.thrust = u1 / param.full_thrust >= 0.7 ? 0.7 : u1 / param.full_thrust;
	...
	uav_utils::limit_range(u.roll_rate, 6.0 * M_PI);
	uav_utils::limit_range(u.pitch_rate, 6.0 * M_PI);
	uav_utils::limit_range(u.yaw_rate, 1.5);
}

void 
Controller::publish_ctrl(const Controller_Output_t& u, const ros::Time& stamp)
{
...
void 
Controller::publish_zero_ctrl(const ros::Time& stamp)
{
	mavros_msgs::AttitudeTarget msg;
...
bool 
Controller::almostZero(const double value) const
{
	return fabs(value) < 0.001; 
}

bool 
Controller::almostZeroThrust(const double thrust_value) const
{
  return fabs(thrust_value) < 0.01;
}

Eigen::Vector3d 
Controller::computeRobustBodyXAxis(
...
Eigen::Vector3d 
Controller::computeFeedBackControlBodyrates(
